...
export class AV {
  ...
  _setupCdnAudio() {
    ...
    try {
      ...
      console.warn('[AV] Failed to set Howler volume:', e.message);
    }
    try {
      Howler.mute(this.muted);
    } catch (e) {
      console.warn('[AV] Failed to mute Howler:', e.message);
    }
    ...
    this.music = new Howl({
      ...
      loop: true,
      volume: this._musicBaseVolume * this._musicVolume,
    });
    if (this.musicOn) {
      try {
        this.music.play();
      } catch (e) {
        console.warn('[AV] Failed to play music:', e.message);
      }
    }
  }
  ...
  _setupFlashEl() {
    ...
    if (!flash) {
      ...
      flash.style.cssText =
        'position:fixed;inset:0;pointer-events:none;background:#fff;opacity:0;z-index:1200;mix-blend-mode:screen;';
  ...
  _initSynthAudio() {
    try {
      ...
      this._windGain = this._ac.createGain();
      ...
      this._engineGain = this._ac.createGain();
      this._engineGain.gain.value = 0;
      this._engineOsc = this._ac.createOscillator();
      this._engineOsc.type = 'sawtooth';
      this._engineOsc.frequency.value = 90;
  ...
  _noiseBurst({ duration = 0.18, vol = 0.25, color = 'low' } = {}) {
    try {
      ...
      for (let i = 0; i < length; i++)
        ch[i] = (Math.random() * 2 - 1) * (color === 'low' ? 0.6 : 0.4);
      const src = this._ac.createBufferSource();
      src.buffer = buf;
      src.loop = false;
      const filt = this._ac.createBiquadFilter();
      filt.type = 'lowpass';
      filt.frequency.value = color === 'low' ? 800 : 2000;
      const g = this._ac.createGain();
      const v = Math.max(0, Math.min(1, vol));
      const now = this._ac.currentTime;
      g.gain.setValueAtTime(0, now);
  ...
  _resumeAudioContexts() {
    const tasks = [];
    const p1 = this._resumeHowlerCtx();
    if (p1) tasks.push(p1);
    const p2 = this._resumeSynthCtx();
    if (p2) tasks.push(p2);
    return tasks;
  }
  ...
  _resumeHowlerCtx() {
    try {
      ...
      if (!globalThis.Howler?.ctx && typeof Ctor === 'function') {
        globalThis.Howler = globalThis.Howler || {};
        globalThis.Howler.ctx = new Ctor();
      }
  ...
  _ensureLoopsAfterUnlock() {
    ...
    if (this.musicOn) {
      if (this.music?.play) {
        ...
        this._startMusicSynth();
      }
    }
  }
  _playConfirmChirp() {
    this._beep({ freq: 660, duration: 0.04, volume: 0.15 });
  }
  ...
  _beep({ freq = 440, duration = 0.08, volume = 0.2 } = {}) {
    try {
      const ac =
        globalThis.Howler?.ctx || new (globalThis.AudioContext || globalThis.webkitAudioContext)();
      if (!ac) return;
  ...
  setMasterVolume01(v) {
    ...
    try {
      if (globalThis.Howler) Howler.volume(this.masterVolume);
    } catch {}
    try {
      if (this._master) this._master.gain.value = this.muted ? 0 : this.masterVolume;
    } catch {}
    try {
      localStorage.setItem('se.volume', String(this.masterVolume));
    } catch (e) {
      console.warn('[AV] Failed to save volume to localStorage:', e.message);
    }
  }
  // Aliases used by UI wiring
  setMasterVolume(v) {
    this.setMasterVolume01(v);
  }
  setSfxVolume(v) {
    ...
    try {
      localStorage.setItem('se.volume.sfx', String(this._sfxVolume));
    } catch {}
  }
  setMusicVolume(v) {
    ...
    try {
      if (this.music?.volume)
        this.music.volume((this._musicBaseVolume || 0.25) * this._musicVolume);
    } catch {}
    try {
      if (this._musicGain)
        this._musicGain.gain.value = this.muted ? 0 : Math.min(0.4, this._musicVolume * 0.4);
    } catch {}
    try {
      localStorage.setItem('se.volume.music', String(this._musicVolume));
    } catch {}
  }
  playMusic() {
    this.musicOn = true;
    try {
      localStorage.setItem('se.musicOn', '1');
      localStorage.setItem('se.music.enabled', 'true');
    } catch {}
    if (this.music?.play) {
      try {
        this.music.play();
  ...
  stopMusic() {
    this.musicOn = false;
    try {
      localStorage.setItem('se.musicOn', '0');
      localStorage.setItem('se.music.enabled', 'false');
    } catch {}
    if (this.music?.stop) {
      try {
        this.music.stop();
  ...
  toggleMute() {
    ...
    try {
      if (globalThis.Howler) Howler.mute(this.muted);
    } catch {}
    try {
      if (this._master) this._master.gain.value = this.muted ? 0 : this.masterVolume;
    } catch {}
    try {
      localStorage.setItem('se.muted', this.muted ? '1' : '0');
    } catch {}
    return this.muted;
  }
  // Music
  toggleMusic() {
    ...
    try {
      localStorage.setItem('se.musicOn', this.musicOn ? '1' : '0');
    } catch {}
    if (this.music?.play) {
      if (this.musicOn) {
        try {
    ...
    if (this.musicOn) this._startMusicSynth();
    else this._stopMusicSynth();
    return this.musicOn;
  }
  _startMusicSynth() {
    try {
  ...
  _stopMusicSynth() {
    ...
    try {
      this._musicGain && (this._musicGain.gain.value = 0);
    } catch {}
  }
  ...
  playExplosion(type, magnitude = 1) {
    if (!this.sounds || Object.keys(this.sounds).length === 0) {
      ...
      this._noiseBurst({
        duration: 0.12 + Math.min(0.2, magnitude * 0.08),
        vol: 0.22 + Math.min(0.2, magnitude * 0.08),
        color,
      });
      return;
    }
    ...
    if (!h) {
      this._fallbackExplosionBeep(type, magnitude);
      return;
    }
    if (h?.state && h.state() !== 'loaded') {
      this._fallbackExplosionBeep(type, magnitude);
    ...
    if (h === this.sounds.explosion_small) {
      h?.volume?.(Math.min(0.6, 0.3 + 0.2 * magnitude));
    }
    const played = (() => {
      try {
        return h?.play?.();
  ...
  _getExplosionSound(type) {
    ...
    if (type === 'heavy' || type === 'bunker' || type === 'mirv' || type === 'funky')
      return s.explosion_heavy;
    return s.explosion_small;
  }
  _fallbackExplosionBeep(type, magnitude) {
    let base;
    if (type === 'nuke') base = 160;
    else if (type === 'heavy') base = 180;
    else base = 220;
    this._beep({
      freq: Math.max(120, base - magnitude * 40),
      duration: 0.1 + Math.min(0.25, magnitude * 0.08),
      volume: 0.25,
    });
  }
  playNukeCharge() {
    this.sounds.nukeCharge?.play();
  }
  playNukeBlast() {
    this.sounds.nukeBlast?.play();
  }
  setWindVolume(strength) {
    ...
    try {
      if (this._windGain) this._windGain.gain.value = this.muted ? 0 : v * 0.15;
    } catch {}
  }
  enginePing() {
    if (!this.engineLoop) {
      ...
      this._ensureContext();
      if (!this._engineGain) this._initSynthAudio();
      try {
  ...
  playUfoFlyby() {
    const h = this.sounds?.ufo_whoosh;
    if (!h) {
      this._beep({ freq: 520, duration: 0.08, volume: 0.15 });
      return;
    }
    if (h?.state && h.state() !== 'loaded') this._beep({ freq: 520, duration: 0.08, volume: 0.15 });
    else {
      const ok = (() => {
        try {
          return h?.play?.();
      ...
      if (ok == null) this._beep({ freq: 520, duration: 0.08, volume: 0.15 });
    }
  }
  playPlaneFlyby() {
    const h = this.sounds?.plane_flyby;
    if (!h) {
      this._beep({ freq: 300, duration: 0.1, volume: 0.15 });
      return;
    }
    if (h?.state && h.state() !== 'loaded') this._beep({ freq: 300, duration: 0.1, volume: 0.15 });
    else {
      const ok = (() => {
        try {
          return h?.play?.();
      ...
      if (ok == null) this._beep({ freq: 300, duration: 0.1, volume: 0.15 });
    }
  }
  playParatrooperDrop() {
    const h = this.sounds?.paratrooper_drop;
    if (!h) {
      this._beep({ freq: 700, duration: 0.05, volume: 0.12 });
      return;
    }
    if (h?.state && h.state() !== 'loaded') this._beep({ freq: 700, duration: 0.05, volume: 0.12 });
    else {
      const ok = (() => {
        try {
          return h?.play?.();
      ...
      if (ok == null) this._beep({ freq: 700, duration: 0.05, volume: 0.12 });
    }
  }
  playParachuteDeploy() {
    const h = this.sounds?.parachute_deploy;
    if (!h) {
      this._beep({ freq: 820, duration: 0.05, volume: 0.12 });
      return;
    }
    if (h?.state && h.state() !== 'loaded') this._beep({ freq: 820, duration: 0.05, volume: 0.12 });
    else {
      const ok = (() => {
        try {
          return h?.play?.();
      ...
      if (ok == null) this._beep({ freq: 820, duration: 0.05, volume: 0.12 });
    }
  }
  playCrateInbound() {
    const h = this.sounds?.crate_inbound;
    if (!h) {
      this._beep({ freq: 960, duration: 0.04, volume: 0.12 });
      return;
    }
    if (h?.state && h.state() !== 'loaded') this._beep({ freq: 960, duration: 0.04, volume: 0.12 });
    else {
      const ok = (() => {
        try {
          return h?.play?.();
      ...
      if (ok == null) this._beep({ freq: 960, duration: 0.04, volume: 0.12 });
    }
  }
  playCratePickup() {
    const h = this.sounds?.crate_pickup;
    if (!h) {
      this._beep({ freq: 640, duration: 0.05, volume: 0.12 });
      return;
    }
    if (h?.state && h.state() !== 'loaded') this._beep({ freq: 640, duration: 0.05, volume: 0.12 });
    else {
      const ok = (() => {
        try {
          return h?.play?.();
      ...
      if (ok == null) this._beep({ freq: 640, duration: 0.05, volume: 0.12 });
    }
  }
  ...
  playBomberSiren() {
    try {
      ...
      if (!ac || !master) {
        this._beep({ freq: 500, duration: 0.2, volume: 0.2 });
        return;
      }
