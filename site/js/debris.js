...
export class DebrisSystem {
  ...
  _brighten(hex, amt) {
    ...
    try {
      ...
      const r = Math.max(
        0,
        Math.min(255, Math.round(Number.parseInt(h.substring(0, 2), 16) * (1 + amt)))
      );
      const g = Math.max(
        0,
        Math.min(255, Math.round(Number.parseInt(h.substring(2, 4), 16) * (1 + amt)))
      );
      const b = Math.max(
        0,
        Math.min(255, Math.round(Number.parseInt(h.substring(4, 6), 16) * (1 + amt)))
      );
