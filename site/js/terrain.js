export class Terrain {
  ...
  resize(newWidth, newHeight) {
    ...
    if (this._movementMinX != null && this._movementMaxX != null && oldWidth > 0) {
      ...
      this._movementMinX = Math.max(
        0,
        Math.min(newWidth - 1, Math.floor(this._movementMinX * scale))
      );
      this._movementMaxX = Math.max(
        0,
        Math.min(newWidth - 1, Math.floor(this._movementMaxX * scale))
      );
    }
  }
  ...
  generate(profile) {
    ...
    if (chosen === 'ocean') {
      ...
      const floorBase = h * 0.8;
      const floorVariation = h * 0.1;
    ...
    if (chosen === 'hilly' || chosen === 'mountain') {
      ...
      for (let k = 0; k < peaks; k++) {
        ...
        const width = Math.floor(
          (segments / (chosen === 'mountain' ? 3 : 5)) * (0.7 + Math.random() * 0.6)
        );
