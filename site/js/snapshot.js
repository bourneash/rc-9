...
export function toSnapshot(game) {
  try {
    ...
    const tanks = Array.isArray(game.tanks)
      ? game.tanks
          .filter(t => !!t)
          .map(t => ({
            ...
            shield:
              t._shield && (t._shield.turnsLeft || 0) > 0
                ? { turnsLeft: t._shield.turnsLeft, factor: t._shield.factor }
                : null,
          }))
      : [];
    const wrecks = (game.wrecks || []).map(w => ({
      x: Math.floor(w.x),
      y: Math.floor(w.y),
      color: w.color || '#222',
    }));
    const smokes = (game.smokeScreens || []).map(s => ({
      x: Math.floor(s.x),
      y: Math.floor(s.y),
      r: Math.floor(s.radius),
      turnsLeft: s.turnsLeft || 0,
    }));
    const mines = (game.mines || []).map(m => ({
      x: Math.floor(m.x),
      y: Math.floor(m.y),
      ownerName: m.owner?.name || null,
      armedIn: Math.max(0, (m.armedAt || 0) - (performance.now?.() || Date.now())),
      radius: m.radius,
      damage: m.damage,
    ...
    const support = (game.supportActors || [])
      ...
      .map(a => ({
        ...
        vx: a.vx,
        targetX: a.targetX,
        droppedCount: a.droppedCount || 0,
        ownerName: a.owner?.name || null,
        bombType: a.bombType || 'nuke',
        bombCount: a.bombCount || 3,
        bombSpacing: a.bombSpacing || 46,
      }));
    return {
      v: 1,
      ts: Date.now(),
...
function applySky(game) {
  try {
    ...
    const defaults = bg.defaults?.[todNow] || {
      stars: todNow === 'night',
      clouds: todNow !== 'night',
      nebula: todNow === 'night',
      sun: todNow !== 'night',
    ...
    if (game.sunEnabled && game.moonEnabled) {
      if (todNow === 'night') game.sunEnabled = false;
      else game.moonEnabled = false;
    }
    game.stars = null;
  } catch {}
}
...
function restoreWrecks(game, snap) {
  game.wrecks = Array.isArray(snap.wrecks)
    ? snap.wrecks.map(w => ({
        x: w.x,
        y: game.terrain.getHeight(Math.max(0, Math.min(game.width - 1, Math.floor(w.x)))),
        vy: 0,
        color: w.color || '#222',
        created: Date.now(),
        nextSmoke: Date.now() + 200,
        char: true,
      }))
    : [];
}
...
function restoreSmokes(game, snap) {
  game.smokeScreens = Array.isArray(snap.smokes)
    ? snap.smokes.map(s => {
        ...
        return {
          x: gx,
          y: gy,
          radius: Math.max(20, s.r || 60),
          turnsLeft: turns,
          initialTurns: Math.max(1, turns),
          _emitTick: 0,
          _graceMs: 600,
        };
      })
...
function restoreMines(game, snap) {
  ...
  for (const m of snap.mines) {
    ...
    game.mines.push({
      ...
      owner,
      armedAt,
      radius: m.radius ?? game.landMineCfg.radius,
      damage: m.damage ?? game.landMineCfg.damage,
      triggerRadius: m.triggerRadius ?? game.landMineCfg.triggerRadius,
      color: m.color || game.landMineCfg.color,
    });
  }
}
