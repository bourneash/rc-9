...
import {
  ...
  planAIMovement as _planAIMovement,
  executeAIMove as _executeAIMove,
  calculateAIShot as _calculateAIShot,
...
export class Game {
  ...
  saveSnapshotToStorage(reason) {
    try {
      ...
      if (reason) {
        try {
          localStorage.setItem('se.lastGame._lastReason', String(reason));
        } catch {}
      }
    } catch (e) {
  ...
  getTestAPI() {
    return {
      ...
      simulateFire: (angle, power, weapon = null) => {
        ...
        if (weapon) {
          this.tanks[this.currentTankIndex].weapon = weapon;
        }
        this.fire();
      },
      ...
      isHealthy: () => {
        ...
        if (this.tanks.length === 0) {
          issues.push('No tanks');
        }
        if (!this.terrain) {
          issues.push('No terrain');
        }
        if (isNaN(this.wind)) {
          issues.push('Wind is NaN');
        }
        if (this.tanks.some(t => isNaN(t.x) || isNaN(t.y))) {
          issues.push('Tank position NaN');
        }
    ...
    try {
      ...
      try {
        ...
        if (gcfg && typeof gcfg === 'object') {
          ...
          if (typeof gcfg.gateInNoHeavy === 'boolean')
            this.airNukes.gateInNoHeavy = gcfg.gateInNoHeavy;
        }
        ...
        if (lmc && typeof lmc === 'object') {
          ...
          if (typeof lmc.triggerRadius === 'number')
            this.landMineCfg.triggerRadius = lmc.triggerRadius;
      ...
      else if (typeof this.config?.graphics?.starCount === 'number')
        this.skyKnobs.stars.count = this.config.graphics.starCount;
      if (typeof starsCfg.brightnessScale === 'number')
        this.skyKnobs.stars.brightnessScale = starsCfg.brightnessScale;
      if (typeof starsCfg.twinkleScale === 'number')
        this.skyKnobs.stars.twinkleScale = starsCfg.twinkleScale;
      if (typeof starsCfg.rotationRate === 'number')
        this.skyKnobs.stars.rotationRate = starsCfg.rotationRate;
      // Nebula
      if (typeof skyCfg.nebulaAlphaScale === 'number')
        this.skyKnobs.nebulaAlphaScale = skyCfg.nebulaAlphaScale;
      // Celestial blending tuning
      const blendCfg = skyCfg.celestial?.blending || {};
      if (typeof blendCfg.threshold === 'number')
        this.celestial._blendThreshold = blendCfg.threshold;
      ...
      if (typeof celCfg.cycleSeconds === 'number')
        this.celestial.cycleSeconds = celCfg.cycleSeconds;
      if (typeof celCfg.moonCycleSeconds === 'number')
        this.celestial.moonCycleSeconds = celCfg.moonCycleSeconds;
      ...
      if (objs.perTypeMax && typeof objs.perTypeMax === 'object')
        this.skyPerTypeMax = { ...this.skyPerTypeMax, ...objs.perTypeMax };
      // Force-disable satellites regardless of config to remove clutter
      if (this.skyPerTypeMax) this.skyPerTypeMax.satellite = 0;
      // Probabilities
      if (objs?.probabilities?.meteor && typeof objs.probabilities.meteor === 'object')
        this.meteorProbByTod = { ...objs.probabilities.meteor };
      ...
      if (m.entryHeightFrac) {
        if (m.entryHeightFrac.min != null)
          this.meteorCfg.entryHeightFracMin = Number(m.entryHeightFrac.min);
        if (m.entryHeightFrac.max != null)
          this.meteorCfg.entryHeightFracMax = Number(m.entryHeightFrac.max);
      }
      ...
      if (m.burst) {
        if (m.burst.chance && typeof m.burst.chance === 'object')
          this.meteorCfg.burst.chance = { ...this.meteorCfg.burst.chance, ...m.burst.chance };
        if (Array.isArray(m.burst.countRange)) {
          this.meteorCfg.burst.countMin = Number(m.burst.countRange[0]);
          this.meteorCfg.burst.countMax = Number(m.burst.countRange[1]);
        } else {
        ...
        if (Array.isArray(m.burst.gapMsRange)) {
          this.meteorCfg.burst.gapMinMs = Number(m.burst.gapMsRange[0]);
          this.meteorCfg.burst.gapMaxMs = Number(m.burst.gapMsRange[1]);
        } else {
      ...
      if (m.burn) {
        const b = m.burn;
        if (b.gravityVyPerFrame != null)
          this.meteorCfg.burn.gravityVyPerFrame = Number(b.gravityVyPerFrame);
        if (b.atmosphere && typeof b.atmosphere === 'object')
          this.meteorCfg.burn.atmo = { ...this.meteorCfg.burn.atmo, ...b.atmosphere };
        if (b.sizeShrinkBase != null) this.meteorCfg.burn.sizeShrinkBase = Number(b.sizeShrinkBase);
        if (b.sizeShrinkSpeedFactor != null)
          this.meteorCfg.burn.sizeShrinkSpeedFactor = Number(b.sizeShrinkSpeedFactor);
        if (b.burnUpSizeThreshold != null)
          this.meteorCfg.burn.burnUpSizeThreshold = Number(b.burnUpSizeThreshold);
        if (b.burnUpHeightMin != null)
          this.meteorCfg.burn.burnUpHeightMin = Number(b.burnUpHeightMin);
        if (b.burnUpProb && typeof b.burnUpProb === 'object')
          this.meteorCfg.burn.burnUpProb = { ...this.meteorCfg.burn.burnUpProb, ...b.burnUpProb };
      }
    } catch (error) {
  ...
  startNewGameWithConfig(cfg) {
    ...
    this.configuredAmmoCounts =
      this.ammoMode === 'custom' && cfg?.ammoCounts ? { ...cfg.ammoCounts } : null;
    ...
    for (let i = 0; i < total; i++) {
      ...
      const shouldUseUnderwaterVehicles =
        this.terrain?._isOceanTerrain ||
  ...
  initializeAmmoForTank(tank, cfg) {
    ...
    if (mode === 'missile-only') {
      tank.ammo.missile = 999;
      return;
    }
    if (mode === 'standard') {
      tank.ammo = { ...standard };
  ...
  spawnTanks(count) {
    ...
    const waterY =
      isOcean && Number.isFinite(this.terrain.waterSurfaceY) ? this.terrain.waterSurfaceY : null;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (isOcean) {
        ...
        if (useShip) {
          ...
          y = Math.max(
            surface + 15,
            Math.min(floorY - 15, minDepth + Math.random() * (maxDepth - minDepth))
          );
  ...
  gameLoop(timestamp) {
    ...
    this.animationFrameId = (globalThis.requestAnimationFrame || requestAnimationFrame)(
  ...
  update(deltaTime) {
    ...
    try {
      ...
      if (cel.enabled && autoTime) {
        ...
        cel.progress =
          (cel.progress + (step / cyc) * sunRate * (this.celestialSpeedScale ?? 1)) % 1;
        const mcyc = Math.max(10, Number(cel.moonCycleSeconds) || 600);
        const moonRate = Math.max(0, Number(this.skySpeeds?.moon ?? 1)) || 1;
        cel.moonPhase =
          (cel.moonPhase + (step / mcyc) * moonRate * (this.celestialSpeedScale ?? 1)) % 1;
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      if (projectile.type === 'napalm') {
        ...
        const canEmit =
          projectile._apexReached ||
          traveledE >= minEmitDistance ||
          (projectile.framesAlive || 0) >= minEmitFrames;
        if (canEmit) {
          ...
          if (!nearOwner) {
            ...
            while (projectile._emitAccum >= emitEveryMs) {
              ...
              if (hz?.nodes && hz.nodes.length < 90) {
                ...
                try {
                  this.particleSystem.createExplosionDownward(px, py, 4, '#ff8a3a');
                } catch {}
              }
            }
      ...
      if (projectile.type === 'parachute_flare' && projectile.vy > 0 && !projectile._deployed) {
        ...
        try {
          document.dispatchEvent(new CustomEvent('game:parachute-deploy'));
        } catch {}
      ...
      if (
        projectile.type === 'cluster' &&
        projectile.vy > 2 &&
        !projectile._split &&
        projectile.framesAlive > 3
      ) {
      ...
      if (
        projectile.x < -offMargin ||
        projectile.x > this.width + offMargin ||
        projectile.y < topLimit ||
        projectile.y > this.height + offMargin
      ) {
        this.projectiles.splice(i, 1);
    ...
    this.particleSystem.update(dt, x =>
      this.terrain
        ? this.terrain.getHeight(Math.max(0, Math.min(Math.floor(x), this.width - 1)))
        : this.height
    );
    ...
    if (!this.gameOver && !this.turnEnding && !this.isAnimating && this.projectiles.length === 0) {
      ...
      if (since >= this.minCrateIntervalMs) {
        ...
        if (Math.random() < p) {
          ...
          try {
            document.dispatchEvent(new CustomEvent('game:crate-inbound'));
          } catch {}
        }
      }
    }
  ...
  render() {
    ...
    const wantSkyObjects =
      typeof f.skyObjects === 'boolean' ? f.skyObjects : this.skyObjectsEnabled;
    ...
    for (let i = 0; i < this.tanks.length; i++) {
      ...
      if (tank.health > 0) {
        ...
        if (
          ...
          this.teams.length === this.tanks.length
        ) {
    ...
    for (let i = this.supportActors.length - 1; i >= 0; i--) {
      ...
      if (a.type === 'paratrooper') {
        if (a.state === 'descending') {
          ...
          const isUnderwater =
            this.terrain._isOceanTerrain &&
            this.terrain.waterSurfaceY != null &&
            a.y > this.terrain.waterSurfaceY;
        ...
        if (a.subtype === 'bomber') {
          ...
          if (a.droppedCount < totalBombs) {
            ...
            const crossedDropPosition =
              (a.vx < 0 && a._lastX > nextDropPos && a.x <= nextDropPos) ||
              (a.vx > 0 && a._lastX < nextDropPos && a.x >= nextDropPos);

            if (crossedDropPosition) {
              console.log(
                `[BOMBER DEBUG] Dropping bomb ${a.droppedCount + 1}/${totalBombs} at x=${a.x}, dropPosition=${nextDropPos}`
              );
          ...
          a.spawnAtX &&
          !a.dropped &&
          ((a.vx < 0 && a.x <= a.spawnAtX) || (a.vx > 0 && a.x >= a.spawnAtX))
        ) {
          ...
          try {
            document.dispatchEvent(new CustomEvent('game:paratrooper-drop'));
          } catch {}
        }
        ...
        if (isUnderwater) {
          ...
          a.vy = Math.min(
            (a.vy ?? 0.1) + (this.parachuteFlareCfg?.descentAccel ?? 0.004) * dt * 0.016,
            this.parachuteFlareCfg?.vyMax ?? 0.6
          );
          a.x += a.vx;
          a.y += a.vy;

          const ground = this.terrain.getHeight(
            Math.max(0, Math.min(this.width - 1, Math.floor(a.x)))
          );
          if (a.y >= ground - 10) {
            a.y = ground - 10;
        ...
        if (a.state === 'descending') {
          ...
          if (isUnderwater) {
            ...
            a.vx = drift;
            a.vy = Math.min(
              (a.vy ?? 0.12) + (this.parachuteFlareCfg?.descentAccel ?? 0.004) * dt * 0.016,
              this.parachuteFlareCfg?.vyMax ?? 0.6
            );
            a.x += a.vx;
            a.y += a.vy;
          ...
          if (t) {
            ...
            try {
              document.dispatchEvent(new CustomEvent('game:crate-pickup'));
            } catch {}
            a.type = 'done';
          } else {
  ...
  // drawTrajectoryGuide extracted to game-physics.js; rebound on Game.prototype below.
  ...
  drawSoloHUD() {
    ...
    const progress =
      this.soloTargetGoal && this.soloTargetGoal > 0
  ...
  getThemeLibrary() {
    const mk = (
      ...
      glow,
      bedrock = '#1a1410',
      bedrockLine = 'rgba(100, 80, 60, 0.3)'
    ) => ({
      gradientStops: stops,
      edgeStrokeColor: edge,
      edgeGlowColor: glow,
      bedrockColor: bedrock,
      bedrockLineColor: bedrockLine,
    });
    return {
      canyon: {
        label: 'Desert Canyon',
  ...
  applyTheme(themeName, timeOfDay) {
    ...
    try {
      ...
      const defaults = (bg.defaults && bg.defaults[todNow]) || {
        stars: todNow === 'night',
        clouds: todNow !== 'night',
        nebula: todNow === 'night',
        sun: todNow !== 'night',
      ...
      const force =
        bg.overrides?.[this.themeName]?.force ||
        bg.overrides?.[this.themeName]?.[todNow]?.force ||
        bg.force?.[this.themeName] ||
        null;
      if (force) {
      ...
      if (knobOverrides) {
        ...
        const base =
          typeof structuredClone === 'function'
            ? structuredClone(this.skyKnobs)
            : JSON.parse(JSON.stringify(this.skyKnobs));
      ...
      if (this.sunEnabled && this.moonEnabled) {
        if (todNow === 'night') this.sunEnabled = false;
        else this.moonEnabled = false;
      }
    } catch {}
  ...
  setThemeOverride(themeNameOrNull) {
    ...
    const label = themeNameOrNull
      ? this.getThemeLibrary()[themeNameOrNull]?.label || themeNameOrNull
      : 'Random';
  ...
  drawSky() {
    ...
    const top = celActive
      ? triBlend(topD, topK, topN, blends)
      : (theme.skies[tod] || theme.skies['day'])[0];
    const bottom = celActive
      ? triBlend(botD, botK, botN, blends)
      : (theme.skies[tod] || theme.skies['day'])[1];
    ...
    if (this.themeName === 'cave') {
      ...
      const rimAlpha = Math.max(
        0,
        Math.min(1, Number(this.config?.graphics?.cave?.rimAlpha ?? 0.06))
      );
  ...
  updateAndDrawSkyObjects(tod) {
      ...
      if (counts.ufo >= 1) {
        ...
        if (!preferType || counts[preferType] < (typeMax[preferType] ?? 1)) {
          ...
          if (this.skyBurst.remaining > 0) {
            ...
            const burstChance =
              this.meteorCfg?.burst?.chance?.[tod] ??
              (tod === 'night' ? 0.06 : tod === 'dusk' ? 0.04 : 0.02);
    ...
    for (let i = this.skyObjects.length - 1; i >= 0; i--) {
      ...
      if (o.type === 'meteor') {
        ...
        const atmo =
          this.meteorCfg?.burn?.atmo?.[tod] ??
          (tod === 'day' ? 1.35 : tod === 'dusk' ? 1.15 : 0.85);
        ...
        const burnProb =
          this.meteorCfg?.burn?.burnUpProb?.[tod] ??
          (tod === 'day' ? 0.75 : tod === 'dusk' ? 0.6 : 0.35);
  ...
  drawNebula() {
    ...
    const celActive = !!(cel?.enabled && this.timeOfDayOverride == null);
    const alphaMul =
      (celActive ? (cel.nebulaAlpha ?? 1) : 1) *
      (this.effectiveKnobs?.nebulaAlphaScale ?? this.skyKnobs?.nebulaAlphaScale ?? 1);
  ...
  drawStars() {
    ...
    if (!this.stars) {
      ...
      const starCount = Math.max(
        0,
        Math.floor(this.effectiveKnobs?.stars?.count ?? this.skyKnobs?.stars?.count ?? 200)
      );
      for (let i = 0; i < starCount; i++) {
        ...
        this.stars.push({
          ...
          brightness:
            (Math.random() * 0.8 + 0.2) *
            (this.effectiveKnobs?.stars?.brightnessScale ??
              this.skyKnobs?.stars?.brightnessScale ??
              1),
          twinkleSpeed:
            (Math.random() * 0.03 + 0.01) *
            (this.effectiveKnobs?.stars?.twinkleScale ?? this.skyKnobs?.stars?.twinkleScale ?? 1),
    ...
    const rotRate =
      this.effectiveKnobs?.stars?.rotationRate ?? this.skyKnobs?.stars?.rotationRate ?? 0.0005;
    ...
    for (let i = 0; i < this.stars.length; i++) {
      ...
      const t2 = Math.sin(
        time * (star.twinkleSpeed2 || star.twinkleSpeed * 1.7) + star.y * 0.19 + 1.3
      );
      ...
      if (sizeNow <= 0.6) {
        ...
        this.ctx.lineWidth = 1;
        this.ctx.beginPath();
        this.ctx.moveTo(ix - 0.8, iy);
        this.ctx.lineTo(ix + 0.8, iy);
        this.ctx.stroke();
        this.ctx.beginPath();
        this.ctx.moveTo(ix, iy - 0.8);
        this.ctx.lineTo(ix, iy + 0.8);
        this.ctx.stroke();
      } else {
        this.ctx.beginPath();
  ...
  drawPlanets() {
    ...
    const tracks = [
      { r: 10, color: '#c7b299', speedMul: 0.25, phase: 0.15, apexY: 0.18, edgeRise: 0.1 },
  ...
  drawCaveVisibilityMask() {
    ...
    const maskAlpha = Math.max(
      0,
      Math.min(1, Number(this.config?.graphics?.cave?.maskAlpha ?? 0.55))
    );
    ...
    const ambFrac = Math.max(
      0.1,
      Math.min(0.8, Number(this.config?.graphics?.cave?.ambientRadiusFrac ?? 0.3))
    );
    const ambR = Math.max(260, this.width * ambFrac);
    const ambientAlpha = Math.max(
      0,
      Math.min(1, Number(this.config?.graphics?.cave?.ambientAlpha ?? 0.16))
    );
  ...
  addLight(x, y, radius, durationMs, color = '#fff6b0') {
    ...
    this.activeLights.push({
      x,
      y,
      radius: Math.max(10, radius || 140),
      created: now,
      durationMs: Math.max(200, durationMs || 4000),
      color,
    });
  }
  pruneExpiredLights() {
    ...
    this.activeLights = this.activeLights.filter(
      L => now - (L.created || now) < (L.durationMs || 0)
    );
  }
  ...
  drawTankWithEffects(tank) {
    ...
    try {
      if (tank._shield && (tank._shield.turnsLeft ?? 0) > 0) {
        ...
        this.ctx.fillStyle = g;
        this.ctx.beginPath();
        this.ctx.arc(tank.x, tank.y - 10, r, 0, Math.PI * 2);
        this.ctx.fill();
        // Subtle rim
        this.ctx.strokeStyle = 'rgba(150,220,255,0.8)';
        this.ctx.lineWidth = 1.5;
        this.ctx.beginPath();
        this.ctx.arc(tank.x, tank.y - 10, r - 1, 0, Math.PI * 2);
        this.ctx.stroke();
        this.ctx.restore();
      }
    } catch {}
  ...
  drawExplosionWithGlow(explosion) {
    ...
    const alpha =
      typeof explosion.currentRadius === 'number' && typeof explosion.maxRadius === 'number'
  ...
  handleImpact(projectile, directHitTank = null) {
      ...
      if (distT <= hitR) {
        ...
        this.explosions.push(
          new Explosion(
            projectile.x,
            projectile.y,
            Math.max(20, projectile.explosionRadius * 0.6),
            0,
            '#44ddff'
          )
        );
        this.particleSystem.createExplosion(
          projectile.x,
          projectile.y,
          Math.max(25, projectile.explosionRadius * 0.8),
          '#44ddff'
        );
        // Progress solo goal
        if (this.soloTargetGoal > 0) {
    ...
    if (projectile.type === 'flare' || projectile.type === 'parachute_flare') {
      ...
      const r =
        projectile.type === 'parachute_flare'
          ? (this.parachuteFlareCfg?.radius ?? 200)
          : (this.flareCfg?.radius ?? 220);
      const d =
        projectile.type === 'parachute_flare'
          ? (this.parachuteFlareCfg?.durationMs ?? 8000)
          : (this.flareCfg?.durationMs ?? 6000);
      const color =
        projectile.type === 'parachute_flare'
          ? (this.parachuteFlareCfg?.color ?? '#fff2a0')
          : (this.flareCfg?.color ?? '#fff6b0');
      this.addLight(projectile.x, gy, r, d, color);
      // Small spark puff
      this.particleSystem.createExplosion(projectile.x, gy, 18, '#ffd966');
      this.addLog(
        projectile.type === 'parachute_flare' ? 'Parachute flare deployed.' : 'Flare deployed.',
        'info'
      );
    ...
    if (projectile.type === 'emp') {
      ...
      this.explosions.push(
        new Explosion(projectile.x, projectile.y, Math.max(24, r * 0.6), 0, '#77e5ff')
      );
      this.particleSystem.createExplosion(
        projectile.x,
        projectile.y,
    ...
    if (projectile.type === 'healing_bomb') {
      ...
      if (healsApplied > 0) {
        this.addLog(
          `Healing bomb restored ${healsApplied} tank${healsApplied > 1 ? 's' : ''}!`,
          'info'
        );
      } else {
    ...
    if (projectile.type === 'shockwave') {
      ...
      this.addLog(
        `Shockwave affected ${tanksAffected} tank${tanksAffected !== 1 ? 's' : ''}!`,
        'info'
      );
    ...
    if (
      projectile.type === 'marker_attack' ||
      projectile.type === 'marker_medic' ||
      projectile.type === 'marker_airstrike' ||
      projectile.type === 'marker_airnukes'
    ) {
      const gx = Math.max(0, Math.min(Math.floor(projectile.x), this.width - 1));
      ...
      const owner =
        projectile.type === 'marker_medic'
          ? nearestAtMarker('marker_medic') || projectile.owner
          : projectile.owner;
      if (projectile.type === 'marker_attack') {
        ...
        this.supportActors.push({
          type: 'plane',
          x: dirLeft ? this.width + 60 : -60,
          y: 80,
          vx: dirLeft ? -60 : 60,
          spawnAtX: projectile.x,
          dropped: false,
          role: 'attack',
          owner,
        });
      ...
      setTimeout(() => {
        try {
          ...
          this.particleSystem.createExplosion(
            projectile.x,
            projectile.y,
            projectile.explosionRadius * 0.8,
            '#8b7355'
          );
    ...
    // Notify AV layer
    try {
      const ev = new CustomEvent('game:explosion', {
        detail: { type: projectile.type, radius: projectile.explosionRadius },
      });
    ...
    if (projectile.type === 'nuke') {
      ...
      this.particleSystem.createExplosionSmoke(
        projectile.x,
        gy,
        Math.max(18, projectile.explosionRadius * 0.7),
        '#bbbbbb'
      );
    }
    ...
    for (let tank of this.tanks) {
      if (tank.health > 0) {
        ...
        if (distance < projectile.explosionRadius) {
          ...
          if (projectile.type === 'nuke') {
            const push =
              Math.max(0, (projectile.explosionRadius - distance) / projectile.explosionRadius) * 6;
  ...
  hasActiveMirvOrFunky() {
    return this.projectiles.some(
      p => (p.type === 'mirv' || p.type === 'funky') && !p.isMirvlet && !p.isFunkylet
    );
  }
  ...
  endTurn() {
    ...
    if (this.mode === 'realtime') {
      ...
      if (
        ...
        this.soloActive &&
        this.soloShotsTotal !== null &&
        this.soloShotsUsed >= this.soloShotsTotal
      ) {
    ...
    if (t.y < targetY - 0.1) {
      ...
      if (t.y >= targetY - 0.01) {
        ...
        if ((t.vy || 0) > 0.6) {
          ...
          try {
            this.spawnDustForTheme?.(this.themeName, t.x, targetY, 4, { sizeScale: 0.6 });
          } catch {}
        } else {
          t.vy = 0;
        }
  ...
  spawnDustForTheme(themeName, x, y, count, options) {
    if (themeName === 'desert' || themeName === 'canyon')
      return this.spawnDesertDust(x, y, count, options);
  ...
  updateSmokeScreens(dtMs) {
    ...
    for (const s of this.smokeScreens) {
      ...
      if (s._emitTick <= 0) {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          this.particleSystem.createSmokePuff(
            s.x + rx,
            s.y - 4 + ry,
    ...
    this.smokeScreens = this.smokeScreens.filter(
      s => (s.turnsLeft || 0) > 0 || (s._graceMs || 0) > 0
    );
  }
  ...
  renderSmokeScreens(ctx) {
    ...
    for (const s of this.smokeScreens) {
      const lifeFrac =
        s.initialTurns && s.initialTurns > 0 ? (s.turnsLeft || 0) / s.initialTurns : 0;
  ...
  checkTeamsOrHumansState() {
    ...
    if (this.mode === 'teams' && this.teams && this.teams.length === this.tanks.length) {
      ...
      if (aliveA && !aliveB) {
        this.gameOver = true;
        this.showWinnerTeam('A');
      } else if (!aliveA && aliveB) {
        this.gameOver = true;
        this.showWinnerTeam('B');
      }
    } else {
      this.checkGameOver();
    }
    ...
    if (this.mode === 'teams' && this.teams) {
      ...
      for (let i = 0; i < this.tanks.length; i++) {
        if (this.tanks[i].health > 0) {
          if (this.teams[i] === 'A') hpA += this.tanks[i].health;
          else hpB += this.tanks[i].health;
        }
      }
      ...
      let best = -1,
        idx = -1;
      for (let i = 0; i < this.tanks.length; i++) {
        if (this.tanks[i].health > best) {
          best = this.tanks[i].health;
          idx = i;
        }
      }
  ...
  isInputBlocked() {
    ...
    return (
      this.isAnimating || this.gameOver || this.turnEnding || this.paused || this.holdingForSupport
    );
  }
  ...
  fire() {
    ...
    if (currentTank.weapon === 'shield') {
      ...
      this.addLog(
        `${currentTank.name} activated a shield! (-${Math.round((1 - factor) * 100)}% damage until next turn)`,
        'info'
      );
      try {
        // Small visual pop
    ...
    if (currentTank.weapon === 'land_mine') {
      ...
      const tooClose = this.mines.some(
        m => m.owner === currentTank && Math.hypot(m.x - gx, m.y - gy) < 10
      );
      if (tooClose) {
      ...
      this.mines.push({
        ...
        y: gy,
        owner: currentTank,
        armedAt: (performance.now?.() || Date.now()) + (this.landMineCfg.armDelayMs || 600),
        radius: this.landMineCfg.radius,
        damage: this.landMineCfg.damage,
        triggerRadius: this.landMineCfg.triggerRadius,
        color: this.landMineCfg.color,
      });
      this.addLog(`${currentTank.name} planted a land mine.`, 'info');
      try {
        this.particleSystem.createSmokePuff(gx, gy - 4, 8);
      } catch {}
    ...
    if (currentTank.weapon === 'smoke_bomb') {
      ...
      if (nearDown && veryLowPower) {
        ...
        try {
          this.particleSystem.createExplosionSmoke(gx, gy, 60, '#c8c8c8');
        } catch {}
        // End turn shortly, no projectile spawned
        this.scheduleEndTurn(400);
        return;
    ...
    try {
      if (this.tracerPreview && this.tracerPreview.owner === currentTank) this.tracerPreview = null;
    } catch {}
    ...
    this.addLog(
      `${currentTank.name} fired ${weaponNames[currentTank.weapon] || 'Unknown'}!`,
      'info'
    );
  ...
  setWeapon(weapon) {
    ...
    if (currentTank && !currentTank.isAI) {
      ...
      if (restrictionReason) {
        this.addLog(restrictionReason, 'warn');
        currentTank.weapon = this.getFallbackWeapon(currentTank, {
          order: [currentTank.weapon, 'missile', 'tracer', 'homing', 'cluster', 'heavy'],
        });
      } else {
  ...
  setDustEnabledOverride(onOrNull) {
    ...
    const label =
      this.dustOverrideEnabled === null ? 'Auto' : this.dustOverrideEnabled ? 'On' : 'Off';
  ...
  _updateUIImmediate() {
    ...
    if (angleInput) {
      angleInput.value = String(((currentTank.angle % 360) + 360) % 360);
    }
    if (powerInput) {
      powerInput.value = currentTank.power;
    }
    ...
    const setAmmoBadge = el => {
      ...
      if (!currentTank.unlimitedAmmo) {
        ...
        else if (n <= 2) el.classList.add('low');
      }
      el.textContent = txt;
    };
    ...
    if (angleVal) {
      angleVal.textContent = (((currentTank.angle % 360) + 360) % 360) + '°';
    }
    if (powerVal) {
      powerVal.textContent = currentTank.power + '%';
    }
  }
  ...
  refillAmmoForTank(tank) {
    ...
    const mode = this.ammoMode || 'unlimited';
    if (mode === 'unlimited') {
      tank.unlimitedAmmo = true;
      return;
    }
    if (mode === 'missile-only') {
      tank.ammo.missile = Math.max(tank.ammo.missile || 0, 999);
      return;
    }
    if (mode === 'standard' || mode === 'no-heavy') {
      ...
      for (const [k, v] of Object.entries(standard)) {
        if (mode === 'no-heavy' && this.heavyWeapons.has(k)) {
          tank.ammo[k] = 0;
          continue;
        }
  ...
  generateSpawnPositions(count) {
    ...
    const isCanyon =
      this.themeName === 'canyon' &&
      typeof this.terrain?._canyonValleyLeft === 'number' &&
      typeof this.terrain?._canyonValleyRight === 'number';
    ...
    while (xs.length < n && tries < attemptsMax) {
      ...
      const rx = isCanyon
        ? pickLeft
          ? margin + Math.random() * Math.max(1, leftMax - margin)
  ...
  updateWindDisplay() {
    ...
    const safeWind =
      typeof displayWind === 'number' && Number.isFinite(displayWind) ? displayWind : 0;
    ...
    if (windArrow) {
      ...
      if (safeWind < -3)
        arrowChar = '⇐'; // Strong left
      else if (safeWind < -0.5)
        arrowChar = '←'; // Moderate left
      else if (safeWind > 3)
        arrowChar = '⇒'; // Strong right
      else if (safeWind > 0.5)
        arrowChar = '→'; // Moderate right
      ...
      if (safeWind < 0) {
        ...
        windArrow.style.textShadow =
          '0 0 8px #00ffff, 0 0 16px rgba(0,255,255,0.8), 0 2px 4px rgba(0,0,0,0.8)';
      } else if (safeWind > 0) {
        windArrow.style.color = '#ff00ff'; // Magenta for right
        windArrow.style.textShadow =
          '0 0 8px #ff00ff, 0 0 16px rgba(255,0,255,0.8), 0 2px 4px rgba(0,0,0,0.8)';
      } else {
        windArrow.style.color = '#ffd700'; // Gold for calm
        windArrow.style.textShadow =
          '0 0 6px #ffd700, 0 0 12px rgba(255,215,0,0.6), 0 2px 4px rgba(0,0,0,0.8)';
      }
    }
  ...
  disableControls() {
    ...
    if (mobileFire) mobileFire.disabled = true;
    const ai = document.getElementById('angle-input');
    if (ai) ai.disabled = true;
    const pi = document.getElementById('power-input');
    if (pi) pi.disabled = true;
    // Disable weapon picker toggle (graphical menu)
    const wgt = document.getElementById('weapon-grid-toggle');
    if (wgt) wgt.disabled = true;
    const drv = document.getElementById('drive-toggle');
    if (drv) drv.disabled = true;
    const drvM = document.getElementById('drive-toggle-modal');
    if (drvM) drvM.disabled = true;
  }
  ...
  enableControls() {
    ...
    const mobileFire = document.getElementById('mobile-fire');
    if (mobileFire) mobileFire.disabled = false;
    const ai = document.getElementById('angle-input');
    ...
    const wgt = document.getElementById('weapon-grid-toggle');
    if (wgt) wgt.disabled = false;
    const drv = document.getElementById('drive-toggle');
    if (drv) drv.disabled = false;
    const drvM = document.getElementById('drive-toggle-modal');
    if (drvM) drvM.disabled = false;
  }
  ...
  setPaused(on, reason = 'user') {
    ...
    if (this.paused && !wasPaused) {
      ...
      if (this.mode !== 'realtime') {
        ...
        if (ct && ct.isAI) this.disableControls();
  ...
  resume(reason = 'user') {
    this.setPaused(false, reason);
  }
  togglePause(reason = 'user') {
    this.setPaused(!this.paused, reason);
  }
    ...
    try {
      ...
      setTimeout(() => {
        ...
        try {
          ...
          console.warn('[resume] performAITurn failed', e);
        }
      }, 250);
    } catch (e) {
  ...
  reset() {
    ...
    try {
      const cfg =
        this.ammoMode === 'custom' ? { ammoCounts: this.configuredAmmoCounts || {} } : null;
      for (const t of this.tanks) {
      ...
      if (currentTank && currentTank.isAI) {
        ...
        setTimeout(() => {
          ...
          try {
            ...
            console.warn('performAITurn failed after reset', e);
          }
        }, 700);
      } else {
        this.enableControls();
    ...
    for (let i = this.hazards.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        for (const n of h.nodes) {
          ...
          if (n.falling) {
            const gNow = this.terrain.getHeight(
              Math.max(0, Math.min(this.width - 1, Math.floor(n.x)))
            );
            if (n.y >= gNow - 1) {
              n.y = gNow - 1;
              n.vy = 0;
      ...
      while (h._tickAcc >= tickEvery) {
        ...
        for (const t of this.tanks) {
          ...
          if (h.type === 'napalm' && h._ownerId != null) {
            const ownerMatch =
              (t.id != null && String(t.id) === String(h._ownerId)) ||
              (t.name && String(t.name) === String(h._ownerId));
  ...
  renderHazards(ctx) {
    ...
    for (const h of this.hazards) {
      if (
        ...
        if (h.type === 'acid') {
          renderMetaballs(ctx, nodes, bounds, {
            cell: 5,
            threshold: 1.0,
            fillStyle: 'rgba(90,255,90,0.9)',
            strokeStyle: 'rgba(20,80,20,0.6)',
          });
          // Toxic glow
          ctx.save();
        ...
        for (let i = 0; i < 3; i++) {
          ...
          ctx.beginPath();
          ctx.moveTo(
            centerX + Math.cos(angle) * symbolSize * 0.25,
        ...
        if (Math.hypot(dx, dy) <= (m.triggerRadius || this.landMineCfg.triggerRadius)) {
          ...
          for (const tank of this.tanks) {
            ...
            if (dist < r) {
              ...
              if (actual > 0)
                this.addLog(`${tank.name} hit by land mine! Damage: ${actual}`, 'hit');
              if (tank.health <= 0) {
                this.addWreck(tank.x, tank.y, tank.color);
                this.checkGameOver();
              }
    ...
    for (const m of this.mines) {
      ...
      if (armed) {
        ...
        ctx.arc(
          0,
          0,
          Math.max(8, (m.triggerRadius || this.landMineCfg.triggerRadius) + 2),
          0,
          Math.PI * 2
        );
        ctx.fill();
  ...
  ensureTanksOnSurface() {
    ...
    for (const t of this.tanks) {
      ...
      if (t.y >= this.terrain.bedrockLevel - 1) {
        try {
          console.warn('[ensureTanksOnSurface] Tank at/under bedrock after snap', {
            x: t.x,
            y: t.y,
            bedrock: this.terrain.bedrockLevel,
          });
        } catch {}
      }
    }
...
// Re-bind extracted victory methods onto Game.prototype so existing call sites
// (this.checkGameOver(), this.showGameOver(), this.showVictoryToast()) work unchanged.
Game.prototype.checkGameOver = function () {
  return _checkGameOver(this);
};
Game.prototype.showGameOver = function (winner) {
  return _showGameOver(this, winner);
};
...
Game.prototype.drawTrajectoryGuide = function () {
  return _drawTrajectoryGuide(this);
};

// Re-bind extracted AI methods onto Game.prototype so existing call sites
// (this.performAITurn(), this.chooseAIWeapon(...), etc.) work unchanged.
Game.prototype.performAITurn = function () {
  return _performAITurn(this);
};
Game.prototype.shouldAIUseShield = function (aiTank, enemyTargets) {
  return _shouldAIUseShield(this, aiTank, enemyTargets);
};
Game.prototype.planAIMovement = function (aiTank, target, distance) {
  return _planAIMovement(this, aiTank, target, distance);
};
Game.prototype.executeAIMove = function (aiTank, plan, onDone) {
  return _executeAIMove(this, aiTank, plan, onDone);
};
Game.prototype.calculateAIShot = function (distance, dx, dy, target) {
  return _calculateAIShot(this, distance, dx, dy, target);
};
