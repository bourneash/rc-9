...
export class Game {
    ...
    try {
      ...
      const afterMove = () => {
        ...
        const {
          ...
          power: bestPower,
          weapon: bestWeapon,
        } = this.calculateAIShot(distance, dx, dy, target);
        // Defensive check: consider using shield instead of attacking
        const aiTargets = this.tanks.filter(
          t => t && t.health > 0 && t !== aiTank && !this.isFriendly(aiTank, t)
        );
  ...
  planAIMovement(aiTank, target, distance) {
    try {
      ...
      else if (aiTank.x > this.width - 60) dir = -1;
      ...
      const step = () => {
        ...
        if (moved) {
          ...
          if (
            ...
            try {
              this.spawnDustForTheme(this.themeName, aiTank.x - dir * 6, aiTank.y - 2, 6);
            } catch {}
          }
          this.updateUI();
          remaining -= 1;
  ...
  calculateAIShot(distance, dx, dy, target) {
    ...
    const skillConfig =
      this.config?.ai?.[aiSkill] || defaultConfigs[aiSkill] || defaultConfigs.medium;
    ...
    try {
      ...
      for (let t = 0; t < 220; t++) {
        vx += windA;
        vy += this.gravityOverride ?? this.gravity;
        x += vx;
        y += vy;
        if (y >= targetDy) {
          fallX = x;
