...
function showErrorNotification(error, title = 'Error') {
  ...
  reloadBtn.style.cssText =
    'margin-top: 12px; padding: 6px 12px; background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.3); border-radius: 4px; color: white; cursor: pointer; font-size: 12px;';
  ...
  closeBtn.style.cssText =
    'background: none; border: none; color: white; font-size: 20px; cursor: pointer; padding: 0; line-height: 1;';
...
(function attachPointerDiagnostics() {
  try {
    ...
    globalThis.__removePointerDiag = () => {
      document.removeEventListener('pointerdown', dbg, { capture: true });
      document.removeEventListener('pointerup', dbg, { capture: true });
      console.info('[diag] pointer diagnostics removed');
    };
    console.info(
      '[diag] pointer diagnostics attached — call globalThis.__removePointerDiag() to remove'
    );
  } catch (e) {
    console.warn('[diag] failed to attach pointer diagnostics', e);
  }
})();
...
(function bootstrapSessionRestore() {
  try {
    ...
    if (localStorage.getItem(UI_RESTORE_PREF_KEY) === null) {
      try {
        localStorage.setItem(UI_RESTORE_PREF_KEY, 'true');
      } catch {}
    }
...
window.addEventListener('beforeunload', () => {
  try {
    game.saveSnapshotToStorage('beforeunload');
  } catch {}
});
...
try {
  ...
  memoryManager.setInterval(() => {
    try {
      game.saveSnapshotToStorage('interval');
    } catch {}
  }, intervalMs);
  ...
  memoryManager.addEventListener(document, 'visibilitychange', () => {
    if (document.hidden) {
      try {
        game.saveSnapshotToStorage('hidden');
      } catch {}
    }
  });
} catch {}
...
try {
  ...
  memoryManager.addEventListener(globalThis, 'blur', () => {
    try {
      game.setPaused(true, 'window-blur');
    } catch {}
  });

  memoryManager.addEventListener(globalThis, 'focus', () => {
    try {
      game.setPaused(false, 'window-blur');
    } catch {}
    ...
    try {
      if (!document.hidden) game.setPaused(false, 'document-hidden');
    } catch {}
  });
  ...
  memoryManager.addEventListener(document, 'visibilitychange', () => {
    if (document.hidden) {
      try {
        game.setPaused(true, 'document-hidden');
      } catch {}
    } else {
      try {
        game.setPaused(false, 'document-hidden');
      } catch {}
      // If the tab becomes visible, treat the window as focused for our purposes.
      try {
        if (document.hasFocus?.()) game.setPaused(false, 'window-blur');
      } catch {}
    }
  });
} catch {}
...
async function initAVOverlay() {
  ...
  try {
    ...
    memoryManager.addEventListener(globalThis, 'touchstart', unlock, {
      capture: true,
      passive: true,
    });
    ...
    try {
      ...
      if (needsUnlock) {
        ...
        btn.style.cssText =
          'position:fixed; right:14px; bottom:60px; z-index:1200; padding:8px 12px; border-radius:8px; background:#333; color:#fff; border:1px solid #666; box-shadow:0 2px 6px rgba(0,0,0,0.25)';
...
async function ensureAVVisuals() {
  ...
  if (!avVisualsLoadPromise) {
    avVisualsLoadPromise = import('./deps-visual.js')
      .then(() => {
        try {
          av?.enableVisuals?.(canvas);
        } catch (error) {
          console.warn('[main] Failed enabling AV visuals:', error);
        }
      })
      .catch(error => {
...
// New audio hooks
document.addEventListener('game:engine-ping', () => {
  av?.unlockAudio?.();
  av?.enginePing?.();
});
document.addEventListener('game:plane-flyby', () => {
  av?.unlockAudio?.();
  av?.playPlaneFlyby?.();
});
document.addEventListener('game:bomber-inbound', () => {
  av?.unlockAudio?.();
  av?.playBomberSiren?.();
});
document.addEventListener('game:ufo-flyby', () => {
  av?.unlockAudio?.();
  av?.playUfoFlyby?.();
});
document.addEventListener('game:paratrooper-drop', () => {
  av?.unlockAudio?.();
  av?.playParatrooperDrop?.();
});
document.addEventListener('game:parachute-deploy', () => {
  av?.unlockAudio?.();
  av?.playParachuteDeploy?.();
});
document.addEventListener('game:crate-inbound', () => {
  av?.unlockAudio?.();
  av?.playCrateInbound?.();
});
document.addEventListener('game:crate-pickup', () => {
  av?.unlockAudio?.();
  av?.playCratePickup?.();
});
...
function activateModalA11y(modal, initialFocusSelector) {
  ...
  modal._restoreFocusEl =
    document.activeElement instanceof HTMLElement ? document.activeElement : null;

  if (!modal._focusTrapHandler) {
    modal._focusTrapHandler = event => {
      if (event.key !== 'Tab') return;
      const focusables = Array.from(modal.querySelectorAll(MODAL_FOCUSABLE_SELECTOR)).filter(
        el => el instanceof HTMLElement && !el.hasAttribute('disabled') && el.tabIndex !== -1
      );
...
function openNewGameModal() {
  ...
  try {
    game.setPaused(true, 'new-game-modal');
  } catch {}
  try {
    saveLastUIState('new-game');
  } catch {}
...
function closeNewGameModal() {
  ...
  try {
    game.setPaused(false, 'new-game-modal');
  } catch {}
  try {
    saveLastUIState('playing');
  } catch {}
...
function bindNewGameModalEvents() {
  ...
  document.getElementById('setup-cancel')?.addEventListener('click', e => {
    e.preventDefault();
    e.stopPropagation();
    closeNewGameModal();
  });
  ...
  function performFullReset() {
    ...
    document.querySelectorAll('#setup-ammo-custom input[type="number"]').forEach(el => {
      el.value = '';
    });
  ...
  document.getElementById('setup-theme')?.addEventListener('change', () => {
    renderThemePreviews();
    persistSetupSelection();
  });
  document.getElementById('setup-time')?.addEventListener('change', () => {
    persistSetupSelection();
  });
  document.getElementById('setup-static-time')?.addEventListener('change', () => {
    persistSetupSelection();
  });
  ...
  document.getElementById('setup-solo-targets')?.addEventListener('input', () => {
    persistSetupSelection();
  });
  ...
  for (const id of ammoIds) {
    document.getElementById(`ammo-${id}`)?.addEventListener('input', () => {
      persistSetupSelection();
    });
  }
  ...
  document.getElementById('skip-continue')?.addEventListener('click', e => {
    e.preventDefault();
    document.getElementById('skip-modal')?.classList.add('hidden');
  });
  document.getElementById('skip-to-result')?.addEventListener('click', e => {
    e.preventDefault();
    document.getElementById('skip-modal')?.classList.add('hidden');
    game.finishAIGameImmediately();
  });
}
...
function collectSetupConfig() {
  ...
  const healthMultiplier = Number.parseInt(
    document.querySelector('input[name="health"]:checked')?.value || '100'
  );
...
function collectTeams(total, mode) {
  ...
  for (let i = 0; i < total && (countA > 4 || countB > 4); i++) {
    if (teams[i] === 'A' && countA > 4) {
      teams[i] = 'B';
      countA--;
      countB++;
    } else if (teams[i] === 'B' && countB > 4) {
      teams[i] = 'A';
      countB--;
      countA++;
    }
...
function renderSlotsUI() {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const styleLeft = document.createElement('button');
    styleLeft.type = 'button';
    styleLeft.textContent = '◀';
    styleLeft.title = 'Previous tank style';
    const styleRight = document.createElement('button');
    styleRight.type = 'button';
    styleRight.textContent = '▶';
    styleRight.title = 'Next tank style';
    const styles = ['classic', 'heavy', 'sleek'];
    const styleSel = document.createElement('input');
    styleSel.type = 'hidden';
    styleSel.className = 'slot-style';
    ...
    styleLeft.addEventListener('click', () => {
      styleIndex = (styleIndex + styles.length - 1) % styles.length;
      styleSel.value = styles[styleIndex];
      renderStyle();
      persist();
    });
    styleRight.addEventListener('click', () => {
    ...
    colorInput.className = 'slot-color';
    colorInput.value =
      prev[i]?.color || stored?.[i]?.color || defaultColors[i % defaultColors.length];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (hidden && count) {
    count.textContent = String(clamp(Number.parseInt(hidden.value || '2')));
  }
  ...
  if (tHidden && tCount) {
    tCount.textContent = String(tClamp(Number.parseInt(tHidden.value || '10')));
  }
...
function bindUI() {
  ...
  function applyDebugPrefs(p) {
    ...
    try {
      // Trajectory guide
      if (trajGuideToggle) {
        trajGuideToggle.checked = !!p.trajectoryGuide;
        game.setTrajectoryGuide(!!p.trajectoryGuide);
      }
      // Theme/time
      if (themeSelect) {
        themeSelect.value = p.theme || 'random';
        game.setThemeOverride(p.theme && p.theme !== 'random' ? p.theme : null);
      }
      if (timeSelect) {
        timeSelect.value = p.time || 'auto';
        game.setTimeOfDayOverride(p.time && p.time !== 'auto' ? p.time : null);
      }
      // Wind override
      if (windOverride) {
        const val = typeof p.windOverride === 'number' ? p.windOverride : 0;
        windOverride.value = String(val);
        const wv = document.getElementById('wind-override-value');
        if (wv) wv.textContent = val === 0 ? 'Off' : val.toFixed(1);
      ...
      if (fuelSlider) {
        ...
        if (fv === 200) {
          ...
          const pct = Math.round((fv / 200) * 100);
          if (label) label.textContent = `${pct}% (${fv})`;
      ...
      if (healthOverride) {
        ...
        const lab = document.getElementById('health-override-value');
        if (lab) lab.textContent = String(hv);
      ...
      if (gravityOverride) {
        ...
        const lab = document.getElementById('gravity-override-value');
        if (lab) lab.textContent = gv.toFixed(2);
      ...
      if (ih) {
        ...
        const lab = document.getElementById('damage-override-value');
        if (lab) lab.textContent = dv.toFixed(1);
      ...
      if (terrainSmoothness) {
        ...
        const lab = document.getElementById('terrain-smoothness-value');
        if (lab) lab.textContent = String(tv);
      ...
      if (p.debris) {
        ...
        const a = Number.parseFloat(p.debris.amount ?? 1);
        game.debrisSystem.amountMultiplier = a;
        const lv = Number.parseInt(p.debris.lifetimeSec ?? 10);
        game.debrisSystem.lifetimeMs = lv * 1000;
        const dav = document.getElementById('debris-amount-value');
        if (dav) dav.textContent = a.toFixed(1);
        const dlv = document.getElementById('debris-lifetime-value');
        if (dlv) dlv.textContent = String(lv);
      }
      // Dust
      if (dustAmount) {
        const v = Number.parseFloat(p.dust?.amount ?? 1);
        dustAmount.value = String(v);
        const lab = document.getElementById('dust-amount-value');
        if (lab) lab.textContent = v.toFixed(1);
        game.setDustAmountMultiplier(v);
      }
      if (dustSize) {
        const v = Number.parseFloat(p.dust?.size ?? 1);
        dustSize.value = String(v);
        const lab = document.getElementById('dust-size-value');
        if (lab) lab.textContent = v.toFixed(1);
        game.setDustSizeScale(v);
      }
      if (dustLife) {
        const v = Number.parseFloat(p.dust?.life ?? 1);
        dustLife.value = String(v);
        const lab = document.getElementById('dust-life-value');
        if (lab) lab.textContent = v.toFixed(1);
      ...
      if (p.highlight) {
        if (highlightEnabled) {
          highlightEnabled.checked = !!p.highlight.enabled;
          game.setActiveHighlightEnabled(!!p.highlight.enabled);
        }
        if (highlightIntensity) {
          ...
          const lab = document.getElementById('highlight-intensity-value');
          if (lab) lab.textContent = hv.toFixed(1);
  ...
  rotationInput?.addEventListener('input', e => {
    ...
    if (
      currentTank &&
      currentTank.type === 'submarine' &&
      typeof currentTank.rotation === 'number'
    ) {
  ...
  const weaponMeta = {
    ...
    bouncing_bomb: {
      name: 'Bouncing Bomb',
      icon: '🏀',
      desc: 'Bounces a few times, then explodes',
    },
    supply_crate: {
      name: 'Supply Crate',
      icon: '🎁',
      desc: 'Call in a supply drop (refills ammo/fuel)',
    },
    ...
    land_mine: {
      name: 'Land Mine',
      icon: '💣',
      desc: 'Place a mine that detonates on enemy contact',
    },
    toxic_gas: {
      name: 'Toxic Gas',
      icon: '☠️',
      desc: 'Creates toxic cloud that damages tanks over time',
    },
  };
  ...
  function renderWeaponMenu() {
    ...
    for (const w of order) {
      ...
      badge.className = 'weapon-ammo';
      if (t.unlimitedAmmo) {
        badge.textContent = '∞';
      } else {
        badge.textContent = String(ammo);
  ...
  function syncWeaponMenuState() {
    ...
    for (const item of weaponMenu.querySelectorAll('.weapon-item')) {
      ...
      item.setAttribute('aria-disabled', disabled ? 'true' : 'false');
      item.title = !allowed
        ? restrictionReason || 'Unavailable right now'
        : isEmpty
          ? 'Out of ammo'
          : '';
  ...
  function openWeaponMenu() {
    ...
    const closeOnOutside = e => {
      ...
      const isToggleClick =
        weaponGridToggle && (e.target === weaponGridToggle || weaponGridToggle.contains(e.target));
  ...
  const toggleWeaponMenu = e => {
    ...
    if (weaponMenu.classList.contains('hidden')) openWeaponMenu();
    else closeWeaponMenu();
  };
  ...
  const chooserMaybeToggle = e => {
    ...
    const insideButton =
      weaponGridToggle && (t === weaponGridToggle || weaponGridToggle.contains(t));
  ...
  try {
    const vol =
      av?.masterVolume === undefined || av?.masterVolume === null
  ...
  bindEdgeTabAction(volumeTab, () => {
    openVolumeModal();
  });
  const restartTab = document.getElementById('restart-tab');
  bindEdgeTabAction(restartTab, () => {
    openRestartModal();
  });
  ...
  function attachVolumeModalHandlers() {
    ...
    masterSlider?.addEventListener('input', e => {
      ...
      try {
        localStorage.setItem('se.volume.master', val);
      } catch {}
    ...
    sfxSlider?.addEventListener('input', e => {
      ...
      try {
        localStorage.setItem('se.volume.sfx', val);
      } catch {}
      // Update audio system if available
    ...
    musicSlider?.addEventListener('input', e => {
      ...
      try {
        localStorage.setItem('se.volume.music', val);
      } catch {}
      // Update audio system if available
    ...
    musicToggleBtn?.addEventListener('click', () => {
      ...
      try {
        localStorage.setItem('se.music.enabled', String(newState));
      } catch {}
      // Update audio system if available
      if (av) {
  ...
  function closeDebugModal() {
    ...
    if (debugMenuOriginalParent) {
      try {
        if (
          debugMenuNextSibling &&
          debugMenuNextSibling.parentElement === debugMenuOriginalParent
        ) {
  ...
  rerollThemeBtn?.addEventListener('click', e => {
    e.preventDefault();
    game.rerollThemeNow();
  });
  rerollTimeBtn?.addEventListener('click', e => {
    e.preventDefault();
    game.rerollTimeNow();
  });
  ...
  refillAmmoCurrentBtn?.addEventListener('click', e => {
    e.preventDefault();
    game.refillAmmo(false);
  });
  refillAmmoAllBtn?.addEventListener('click', e => {
    e.preventDefault();
    game.refillAmmo(true);
  });
  // Highlight controls
  ...
  resetCheats?.addEventListener('click', e => {
    ...
    const wv = document.getElementById('wind-override-value');
    if (wv) wv.textContent = 'Off';
    setVal('fuel-slider', 200);
    const fv = document.getElementById('fuel-value-display');
    if (fv) fv.textContent = '100% (Normal)';
    setVal('health-override-slider', 100);
    const hv = document.getElementById('health-override-value');
    if (hv) hv.textContent = '100';
    setVal('gravity-override-slider', 0.3);
    const gv = document.getElementById('gravity-override-value');
    if (gv) gv.textContent = '0.30';
    setVal('damage-override-slider', 1);
    const dv = document.getElementById('damage-override-value');
    if (dv) dv.textContent = '1.0';
    setVal('terrain-smoothness-slider', 50);
    const tv = document.getElementById('terrain-smoothness-value');
    if (tv) tv.textContent = '50';
    const de = document.getElementById('debris-enabled');
    if (de) de.checked = true;
    setVal('debris-amount', 1);
    const dav = document.getElementById('debris-amount-value');
    if (dav) dav.textContent = '1.0';
    setVal('debris-lifetime', 10);
    const dlv = document.getElementById('debris-lifetime-value');
    if (dlv) dlv.textContent = '10';
    const dusE = document.getElementById('dust-enabled');
    if (dusE) dusE.checked = true;
    setVal('dust-amount', 1);
    const dusA = document.getElementById('dust-amount-value');
    if (dusA) dusA.textContent = '1.0';
    setVal('dust-size', 1);
    const dusS = document.getElementById('dust-size-value');
    if (dusS) dusS.textContent = '1.0';
    setVal('dust-life', 1);
    const dusL = document.getElementById('dust-life-value');
    if (dusL) dusL.textContent = '1.0';
  ...
  try {
    applyDebugPrefs(loadDebugPrefs());
  } catch {}
}
...
function tryBindOnce() {
  if (uiBound) return;
  const hasEssentialControls =
    document.getElementById('weapon-grid-toggle') ||
  ...
  try {
    bindUI();
    console.info('[ui] controls bound');
  } catch (e) {
    console.warn('[ui] bind failed:', e);
  }
}
...
window.addEventListener('load', () => {
  ...
  if (!uiBound) {
    ...
    try {
      bindUI();
      console.info('[ui] bound via final load fallback');
    } catch (e) {
      console.error('[ui] final load bind failed', e);
    }
  }
});
...
function ensureBoundOnInteractionOnce() {
  if (!uiBound) {
    try {
      ...
      console.warn('[ui] late bind on interaction failed', e);
    }
  }
}
document.addEventListener('pointerdown', ensureBoundOnInteractionOnce, {
  capture: true,
  once: true,
});
document.addEventListener('click', ensureBoundOnInteractionOnce, { capture: true, once: true });
document.addEventListener('touchstart', ensureBoundOnInteractionOnce, {
  capture: true,
  once: true,
  passive: true,
});
...
function isGameInputBlocked() {
  ...
  return (
    game.gameOver || game.isAnimating || game.paused || game.turnEnding || game.holdingForSupport
  );
}
  ...
  if (joystick && stick) {
    ...
    function move(e) {
      ...
      if (!game.isAnimating && !game.paused && !game.gameOver) {
        if (game.driveMode) handleDrive(nx);
        else handleAim(nx, ny);
      }
